...
void WindowDataLayer<Dtype>::InternalThreadEntry() {
  ...
  bool shuffle = this->layer_param_.window_data_param().shuffle();
  //bool is_mask = this->layer_param_.window_data_param().shuffle();
...
void WindowDataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  //bool is_mask = this->layer_param_.window_data_param().is_mask();
