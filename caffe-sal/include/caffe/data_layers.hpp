...
template <typename Dtype>
class MCWindowDataLayer : public Layer<Dtype>, public InternalThread {
 public:
  explicit MCWindowDataLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual ~MCWindowDataLayer();
  virtual void SetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);

  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_MCWINDOW_DATA;
  }
  virtual inline int ExactNumBottomBlobs() const { return 0; }
 ...
 protected:
  ...
  shared_ptr<Caffe::RNG> prefetch_rng_;
  Blob<Dtype> prefetch_data_;
  Blob<Dtype> prefetch_mask_;
  Blob<Dtype> prefetch_label1_;
  //Blob<Dtype> prefetch_label2_;
  Blob<Dtype> data_mean_;
  vector<std::pair<std::string, vector<int> > > image_database_;
  vector<vector<std::pair<std::string, vector<int> > > > mask_database_;
  enum WindowField { IMAGE_INDEX, LABEL, OVERLAP, X1, Y1, X2, Y2, NUM };
  vector<vector<float> > fg_windows_;
  vector<vector<float> > bg_windows_;
  vector<vector<float> > all_windows_;
...
template <typename Dtype>
class NAMCWindowDataLayer : public Layer<Dtype>, public InternalThread {
 public:
  explicit NAMCWindowDataLayer(const LayerParameter& param)
      : Layer<Dtype>(param) {}
  virtual ~NAMCWindowDataLayer();
  virtual void SetUp(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top);

  virtual inline LayerParameter_LayerType type() const {
    return LayerParameter_LayerType_NAMCWINDOW_DATA;
  }
  virtual inline int ExactNumBottomBlobs() const { return 0; }
  virtual inline int ExactNumTopBlobs() const { return 3; }
