...
void Net<Dtype>::Init(const NetParameter& in_param) {
  ...
  for (int layer_id = 0; layer_id < param.layers_size(); ++layer_id) {
    ...
    //LOG(INFO) << "bug at: " << layer_names_[layer_id];
  ...
  // LOG(INFO) << "net output size: " << net_output_blobs_.size();
...
void Net<Dtype>::AppendTop(const NetParameter& param, const int layer_id,
  ...
  if (layer_param && layer_param->bottom_size() > top_id &&
    ...
    // LOG(INFO) << "debug-at:" << layer_param->bottom_size();
...
Dtype Net<Dtype>::ForwardFromTo(int start, int end) {
  ...
  for (int i = start; i <= end; ++i) {
    ...
    // LOG(INFO) << "Forwarding " << layer_names_[i] << ", loss = " << layer_loss;
    loss += layer_loss;
...
const vector<Blob<Dtype>*>& Net<Dtype>::ForwardPrefilled(Dtype* loss) {
  // LOG(INFO) << "output layer name: "<<layer_names_[layers_.size() - 1];
  if (loss != NULL) {
  ...
  //   LOG(INFO) << i << "-th output size: " << net_output_blobs_[i]->num() << "," << net_output_blobs_[i]->channels();
  // }
...
void Net<Dtype>::BackwardFromTo(int start, int end) {
  ...
  for (int i = start; i >= end; --i) {
    if (layer_need_backward_[i]) {
      ...
      // LOG(INFO) << "Backwarding " << layer_names_[i];
