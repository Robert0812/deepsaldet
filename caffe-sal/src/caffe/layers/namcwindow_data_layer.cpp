...
void NAMCWindowDataLayer<Dtype>::InternalThreadEntry() {
  ...
  const Dtype scale = this->layer_param_.namcwindow_data_param().scale();
  const int batch_size = this->layer_param_.namcwindow_data_param().batch_size();
  const int crop_size = this->layer_param_.namcwindow_data_param().crop_size();
  const int context_pad = this->layer_param_.namcwindow_data_param().context_pad();
  const bool mirror = this->layer_param_.namcwindow_data_param().mirror();
  const float fg_fraction =
      this->layer_param_.namcwindow_data_param().fg_fraction();
  ...
  const string& crop_mode = this->layer_param_.namcwindow_data_param().crop_mode();
  bool shuffle = this->layer_param_.namcwindow_data_param().shuffle();
  //bool is_mask = this->layer_param_.namcwindow_data_param().shuffle();
  ...
  for (int is_fg = 0; is_fg < 2; ++is_fg) {
    for (int dummy = 0; dummy < num_samples[is_fg]; ++dummy) {
      ...
      pair<std::string, vector<int> > image =
          image_database_[window[NAMCWindowDataLayer<Dtype>::IMAGE_INDEX]];
      ...
      int x1 = window[NAMCWindowDataLayer<Dtype>::X1];
      int y1 = window[NAMCWindowDataLayer<Dtype>::Y1];
      int x2 = window[NAMCWindowDataLayer<Dtype>::X2];
      int y2 = window[NAMCWindowDataLayer<Dtype>::Y2];
      ...
      // crop images
      //cv::Mat cv_img_both(cv_img.size(), cv_img.type());
      //cv_img_both = cv_img.clone();
      //LOG(INFO) << "cv_img_both depth: " << cv_img_both.depth();
      
      //LOG(INFO) << "cv_img depth: " << cv_img.depth();
      cv::Rect roi(x1, y1, x2-x1+1, y2-y1+1);
      cv::Mat cv_cropped_img = cv_img(roi);
      //cv_cropped_img.convertTo(cv_cropped_img, cv_img.type());
      //LOG(INFO) << "cv_cropped_img depth: " << cv_cropped_img.depth();
      cv::resize(cv_cropped_img, cv_cropped_img,
          cv_crop_size, 0, 0, cv::INTER_LINEAR);

      int mid_x = (int)((x1+x2)/2.0);
      int mid_y = (int)((y1+y2)/2.0);
      int w_local = (int)((x2-x1+1)/6.0);
      int h_local = (int)((y2-y1+1)/6.0);

      //LOG(INFO) << "test roi: x1:" << mid_x-w_half << " x2: " << m
      cv::Rect mask_roi(mid_x - w_local, mid_y - h_local, 2*w_local, 2*h_local);
      //cv::Rect mask_roi(x1, y1, x2-x1+1, y2-y1+1);
      cv::Mat cv_cropped_msk = cv_img(mask_roi);
      ...
      if (do_mirror) {
        ...
        cv::flip(cv_cropped_msk, cv_cropped_msk, 1);
      }
        ...
        for (int h = 0; h < cv_cropped_img.rows; ++h) {
          for (int w = 0; w < cv_cropped_img.cols; ++w) {
            ...
            top_data[((item_id * channels + c) * crop_size + h + pad_h)
                     * crop_size + w + pad_w]
                = (pixel - mean[(c * mean_height + h + mean_off + pad_h)
      ...
      for (int c = 0; c < channels; ++c) {
        for (int h = 0; h < cv_cropped_msk.rows; ++h) {
          for (int w = 0; w < cv_cropped_msk.cols; ++w) {
            Dtype pixel =
                static_cast<Dtype>(cv_cropped_msk.at<cv::Vec3b>(h, w)[c]);

            top_mask[((item_id * channels + c) * crop_size + h + pad_h)
                     * crop_size + w + pad_w]
                = (pixel - mean[(c * mean_height + h + mean_off + pad_h)
                           * mean_width + w + mean_off + pad_w])
                  * scale;
      ...
      // get window label
      top_label[item_id] = window[NAMCWindowDataLayer<Dtype>::LABEL];
      ...
      inf << image.first << std::endl
          << window[NAMCWindowDataLayer<Dtype>::X1]+1 << std::endl
          << window[NAMCWindowDataLayer<Dtype>::Y1]+1 << std::endl
          << window[NAMCWindowDataLayer<Dtype>::X2]+1 << std::endl
          << window[NAMCWindowDataLayer<Dtype>::Y2]+1 << std::endl
          << do_mirror << std::endl
...
template <typename Dtype>
NAMCWindowDataLayer<Dtype>::~NAMCWindowDataLayer<Dtype>() {
  JoinPrefetchThread();
}

template <typename Dtype>
void NAMCWindowDataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  LOG(INFO) << "Window data layer:" << std::endl
      ...
      << this->layer_param_.namcwindow_data_param().fg_threshold() << std::endl
      << "  background (non-object) overlap threshold: "
      << this->layer_param_.namcwindow_data_param().bg_threshold() << std::endl
      << "  foreground sampling fraction: "
      << this->layer_param_.namcwindow_data_param().fg_fraction();

  std::ifstream infile(this->layer_param_.namcwindow_data_param().source().c_str());
  CHECK(infile.good()) << "Failed to open window file "
      << this->layer_param_.namcwindow_data_param().source() << std::endl;

  //bool is_mask = this->layer_param_.namcwindow_data_param().is_mask();
  //LOG(INFO) << "is_mask: " << is_mask; 
  ...
  do {
    ...
    const float fg_threshold =
        this->layer_param_.namcwindow_data_param().fg_threshold();
    const float bg_threshold =
        this->layer_param_.namcwindow_data_param().bg_threshold();
    for (int i = 0; i < num_windows; ++i) {
      int label, x1, y1, x2, y2;
      float overlap;
      infile >> label >> overlap >> x1 >> y1 >> x2 >> y2;

      vector<float> window(NAMCWindowDataLayer::NUM);
      window[NAMCWindowDataLayer::IMAGE_INDEX] = image_index;
      window[NAMCWindowDataLayer::LABEL] = label;
      window[NAMCWindowDataLayer::OVERLAP] = overlap;
      window[NAMCWindowDataLayer::X1] = x1;
      window[NAMCWindowDataLayer::Y1] = y1;
      window[NAMCWindowDataLayer::X2] = x2;
      window[NAMCWindowDataLayer::Y2] = y2;

      // add window to foreground list or background list and also add window to
      // a unified list
      if (overlap >= fg_threshold) {
        int label = window[NAMCWindowDataLayer::LABEL];
        CHECK_GT(label, 0);
        ...
        window[NAMCWindowDataLayer::LABEL] = 0;
        window[NAMCWindowDataLayer::OVERLAP] = 0;
  ...
  LOG(INFO) << "Amount of context padding: "
      << this->layer_param_.namcwindow_data_param().context_pad();

  LOG(INFO) << "Crop mode: "
      << this->layer_param_.namcwindow_data_param().crop_mode();

  // image
  int crop_size = this->layer_param_.namcwindow_data_param().crop_size();
  ...
  prefetch_mask_.Reshape(batch_size, channels, crop_size, crop_size);
  ...
  if (this->layer_param_.namcwindow_data_param().has_mean_file()) {
    const string& mean_file =
        this->layer_param_.namcwindow_data_param().mean_file();
...
template <typename Dtype>
void NAMCWindowDataLayer<Dtype>::CreatePrefetchThread() {
  const bool prefetch_needs_rand =
      this->layer_param_.namcwindow_data_param().mirror() ||
      this->layer_param_.namcwindow_data_param().crop_size();
...
template <typename Dtype>
void NAMCWindowDataLayer<Dtype>::JoinPrefetchThread() {
  CHECK(!WaitForInternalThreadToExit()) << "Pthread joining failed.";
}

template <typename Dtype>
unsigned int NAMCWindowDataLayer<Dtype>::PrefetchRand() {
  CHECK(prefetch_rng_);
...
template <typename Dtype>
Dtype NAMCWindowDataLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      vector<Blob<Dtype>*>* top) {
...
#ifdef CPU_ONLY
STUB_GPU_FORWARD(NAMCWindowDataLayer, Forward);
#endif

INSTANTIATE_CLASS(NAMCWindowDataLayer);
